...
async function showHardware() {
  ...
  const usbPanel = usbRole.supported ? `<section class="usb-role-panel"><div class="usb-role-heading"><div><span>USB DATA PORT ROLE</span><b>${roleLabel}</b></div>${usbRole.reboot_required ? '<em>REBOOT PENDING</em>' : '<em>BOOT CONFIG</em>'}</div><p>The Pi Zero has one OTG controller. HID/device mode and USB-host dongles cannot operate at the same time.</p><div class="usb-role-map"><span class="${usbRole.configured === 'hid' ? 'selected' : ''}"><b>HID / OTG</b><small>Connect the data port to a computer and permit USB gadget payloads.</small></span><span class="${usbRole.configured === 'host' ? 'selected' : ''}"><b>USB HOST</b><small>Enumerate a Wi-Fi dongle through a proper OTG host adapter or powered hub.</small></span></div><div class="usb-role-actions"><button type="button" class="usb-role" data-role="hid" ${usbRole.configured === 'hid' ? 'disabled' : ''}>ENABLE HID / OTG</button><button type="button" class="usb-role" data-role="host" ${usbRole.configured === 'host' ? 'disabled' : ''}>ENABLE USB HOST</button></div><small class="usb-role-path">${escapeHtml(usbRole.config_path)} · reboot required after a change</small></section>` : '';
...
$('hardwareBody').onclick = async event => {
  ...
  if (usbRoleButton) {
    ...
    if (!confirm(`Configure ${description}\n\nThe Pi must reboot before the role changes.`)) return;
    ...
    if (response.ok) setTimeout(showHardware, 700); else document.querySelectorAll('.usb-role').forEach(button => { button.disabled = false; });
    return;
  }
