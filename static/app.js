...
async function showHardware() {
  ...
  const usbRole = data.usb_role || {};
  const wirelessInterfaces = (data.interfaces || []).filter(item => item.wireless);
  const roleLabel = usbRole.configured === 'host' ? 'USB HOST' : usbRole.configured === 'hid' ? 'HID / OTG' : 'UNKNOWN';
  const usbPanel = usbRole.supported ? `<section class="usb-role-panel"><div class="usb-role-heading"><div><span>USB DATA PORT ROLE</span><b>${roleLabel}</b></div>${usbRole.reboot_required ? '<em>REBOOT PENDING</em>' : '<em>BOOT CONFIG</em>'}</div><p>The Pi Zero has one OTG controller. HID/device mode and USB-host dongles cannot operate at the same time. Changing this setting immediately reboots the Pi and disconnects this web session.</p><div class="usb-role-map"><span class="${usbRole.configured === 'hid' ? 'selected' : ''}"><b>HID / OTG</b><small>Connect the data port to a computer and permit USB gadget payloads.</small></span><span class="${usbRole.configured === 'host' ? 'selected' : ''}"><b>USB HOST</b><small>Enumerate a Wi-Fi dongle through a proper OTG host adapter or powered hub.</small></span></div><div class="usb-role-actions"><button type="button" class="usb-role" data-role="hid" ${usbRole.configured === 'hid' ? 'disabled' : ''}>ENABLE HID / OTG + REBOOT</button><button type="button" class="usb-role" data-role="host" ${usbRole.configured === 'host' ? 'disabled' : ''}>ENABLE USB HOST + REBOOT</button></div><small class="usb-role-path">${escapeHtml(usbRole.config_path)} · role takes effect during the automatic reboot</small></section>` : '';
  $('hardwareBody').innerHTML = `<div class="system-vitals"><div><b>${escapeHtml(system.hostname)}</b><small>HOST</small></div><div><b>${system.temperature_c == null ? '—' : `${system.temperature_c}°C`}</b><small>CPU</small></div><div><b>${formatBytes(system.memory?.available)}</b><small>RAM FREE</small></div><div><b>${formatBytes(system.disk?.free)}</b><small>DISK FREE</small></div></div><div class="hardware-flags"><span class="${system.bluetooth ? 'ok' : ''}">BT</span><span class="${system.gps ? 'ok' : ''}">GPS</span><span class="${system.sdr ? 'ok' : ''}">SDR</span><span class="${system.nfc ? 'ok' : ''}">NFC</span></div>${usbPanel}<h3>WI-FI INTERFACES</h3>${wirelessInterfaces.map(item => `<div class="interface-row ${item.default_route ? 'protected' : ''}"><div><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.driver || 'virtual')} · ${escapeHtml(item.mode || 'mode unknown')}</small></div><div><span class="admin-state ${item.admin_up ? 'ok' : ''}">${item.admin_up ? 'ENABLED' : 'DISABLED'}</span><span class="connection-state ${item.state === 'up' ? 'ok' : ''}">${item.state === 'up' ? 'CONNECTED' : item.state === 'dormant' ? 'DORMANT' : 'DISCONNECTED'}</span><small>${escapeHtml((item.addresses || []).join(', ') || item.mac || 'no address')}</small></div>${item.default_route ? '<em>PROTECTED ROUTE · CONTROLS LOCKED</em>' : `<div class="interface-actions"><button type="button" class="interface-link" data-interface="${escapeHtml(item.name)}" data-state="${item.admin_up ? 'down' : 'up'}">BRING ${item.admin_up ? 'DOWN' : 'UP'}</button><button type="button" class="interface-mode" data-interface="${escapeHtml(item.name)}" data-mode="${item.mode === 'monitor' ? 'managed' : 'monitor'}">${item.mode === 'monitor' ? 'DISABLE MONITOR' : 'ENABLE MONITOR'}</button></div>`}</div>`).join('') || '<div class="empty-state">No Wi-Fi interfaces detected.</div>'}<div id="hardwareStatus" role="status" aria-live="polite"></div>`;
}

$('hardwareBody').onclick = async event => {
  const usbRoleButton = event.target.closest('.usb-role');
  if (usbRoleButton) {
    const role = usbRoleButton.dataset.role;
    const description = role === 'host' ? 'USB HOST mode? HID gadget payloads will be unavailable after reboot.' : 'HID / OTG mode? USB dongles will not enumerate through the data port after reboot.';
    if (!confirm(`Configure ${description}\n\nWARNING: City Pop will reboot the Pi immediately and this web session will disconnect.`)) return;
    document.querySelectorAll('.usb-role').forEach(button => { button.disabled = true; });
    const response = await fetch('/api/hardware/usb-role', {method: 'POST', headers: authHeaders(true), body: JSON.stringify({role})});
    const data = await response.json();
    $('hardwareStatus').textContent = data.detail || data.error || 'USB role change failed.';
    if (response.ok) $('hardwareStatus').textContent = 'USB role saved. Pi rebooting now — reconnect after it starts.'; else document.querySelectorAll('.usb-role').forEach(button => { button.disabled = false; });
    return;
  }
