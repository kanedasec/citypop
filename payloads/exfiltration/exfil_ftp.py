...
def main():
 ...
 threading.Timer(seconds,server.close_all).start()
 print(f'FTP endpoint: ftp://{primary_ip()}:{port}/',flush=True)
 print(f'Username: {user} · Mode: read-only · Duration: {seconds}s',flush=True)
