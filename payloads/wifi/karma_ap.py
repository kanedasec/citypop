...
def _select_interface_cli(iface_type="wifi"):
    ...
    choice = request_input("Select an unused/unconnected monitor/AP-capable external Wi-Fi adapter — it will be reconfigured and must not carry City Pop management access: ").strip()
    try:
        idx = int(choice)
        if 0 <= idx < len(ifaces):
