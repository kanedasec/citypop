...
def main():
    ...
    _iface=str(request_input("Select an unused/unconnected monitor/AP-capable external Wi-Fi adapter — it will be reconfigured and must not carry City Pop management access",input_type="select",choices=choices)); duration=min(3600,max(10,int(sys.argv[1]) if len(sys.argv)>1 else 300))
    try:
        _do_scan()
