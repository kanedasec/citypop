...
def main():
    interfaces=sorted(p.name for p in Path('/sys/class/net').iterdir() if p.name!='lo'); iface=str(request_input('Select connected capture interface — it must observe the authorized TCP traffic; monitor mode is not required',input_type='select',choices=interfaces))
